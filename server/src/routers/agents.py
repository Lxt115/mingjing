...
async def list_agents(db: AsyncSession = Depends(get_db)):
    data = await services.agents.list_agents(db)
    return ApiResponse(data=data, timestamp=time.time())


@router.get("/{agent_id}")
async def get_agent(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    agent = await services.agents.get_agent(db, agent_id)
    if not agent:
...
async def create_agent(body: AgentCreate, db: AsyncSession = Depends(get_db)):
    agent = await services.agents.create_agent(db, body)
    return ApiResponse(data=agent, timestamp=time.time())


@router.put("/{agent_id}")
async def update_agent(agent_id: uuid.UUID, body: AgentUpdate, db: AsyncSession = Depends(get_db)):
    agent = await services.agents.update_agent(db, agent_id, body)
    if not agent:
        raise HTTPException(status_code=404, detail="角色不存在")
    return ApiResponse(data=agent, timestamp=time.time())
...
async def delete_agent(agent_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ok = await services.agents.delete_agent(db, agent_id)
    if not ok:
        raise HTTPException(status_code=404, detail="角色不存在")
