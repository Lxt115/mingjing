...
async def chat(body: ChatRequest, db: AsyncSession = Depends(get_db)):
    result = await services.pipeline.chat_pipeline(db, body.text, body.agent_id, body.conversation_id)
    if "error" in result:
...
@router.post("/speech")
async def speech(body: SpeechRequest, db: AsyncSession = Depends(get_db)):
    raw = body.audio
    if "," in raw:
        raw = raw.split(",")[1]
    audio_bytes = base64.b64decode(raw)

    result = await services.pipeline.speech_pipeline(db, audio_bytes, body.audio_format, body.agent_id, body.conversation_id)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return ApiResponse(data=ChatPipelineResponse(**result).model_dump(by_alias=True), timestamp=time.time())
