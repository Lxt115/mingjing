...
<template>
  <!-- Desktop Layout -->
  <div v-if="!isMobile" class="animate-[fadeIn_.25s_ease] grid grid-cols-[320px_1fr] gap-5 items-start">
    <div>
  ...
  <div v-else class="flex flex-col h-full">
    <div class="profile-hero bg-gradient-to-br from-[var(--indigo)] to-[var(--teal)] pt-7 pb-8 px-5 relative overflow-hidden">
      <div class="absolute -top-10 -right-10 w-[180px] h-[180px] rounded-full bg-white/10 pointer-events-none"></div>
      <div class="flex items-center gap-3.5 mb-5 relative z-[1]">
        <div class="w-16 h-16 rounded-[22px] bg-white/20 flex items-center justify-center text-[32px] border-[2px] border-white/40">
          {{ user.profile.avatarEmoji }}
        </div>
        <div>
          <div class="text-xl font-black text-white">{{ user.profile.name }}</div>
          <div class="text-xs text-white/70 mt-0.5 font-semibold">ID: {{ user.profile.userId }}</div>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="section-label text-xs font-extrabold text-[var(--text3)] tracking-[.8px] uppercase pt-5 pb-2 px-4">设备与绑定</div>
      <div class="card-mobile bg-[var(--surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] mx-4 mb-3 overflow-hidden">
        <div class="row-item flex items-center justify-between px-4 py-3.5 border-b border-[var(--border)] cursor-pointer active:bg-[var(--bg)]" @click="navigateTo('/devices')">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#e8fdf5] flex items-center justify-center text-[15px] shrink-0">📱</div>
            <div>
              <div class="text-sm font-bold text-[var(--text1)]">管理设备</div>
              <div class="text-xs text-[var(--text3)] mt-px">已绑定 {{ user.profile.boundDeviceCount }} 台 · 1 台在线</div>
            </div>
          </div>
          <div class="flex items-center gap-1.5 text-[var(--text3)] text-sm">
            <span class="text-[10px] font-extrabold bg-[#e8fdf5] text-[var(--teal)] px-2 py-0.5 rounded-[20px]">{{ user.profile.boundDeviceCount }} 台</span>
            <span>›</span>
          </div>
        </div>
        <div class="row-item flex items-center justify-between px-4 py-3.5 cursor-pointer active:bg-[var(--bg)]" @click="ui.openModal('modal-add-device')">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#fffbec] flex items-center justify-center text-[15px] shrink-0">➕</div>
            <div>
      ...
      <div class="section-label text-xs font-extrabold text-[var(--text3)] tracking-[.8px] uppercase pt-4 pb-2 px-4">声音</div>
      <div class="card-mobile bg-[var(--surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] mx-4 mb-3 overflow-hidden">
        <div class="row-item flex items-center justify-between px-4 py-3.5 cursor-pointer active:bg-[var(--bg)]" @click="navigateTo('/voice-library')">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#f0f4ff] flex items-center justify-center text-[15px] shrink-0">🔊</div>
            <div>
              <div class="text-sm font-bold text-[var(--text1)]">声音库</div>
              <div class="text-xs text-[var(--text3)] mt-px">系统音色 · 克隆我的声音 · 声纹识别</div>
            </div>
          </div>
          <span class="text-[var(--text3)] text-sm">›</span>
        </div>
      ...
      <div class="section-label text-xs font-extrabold text-[var(--text3)] tracking-[.8px] uppercase pt-4 pb-2 px-4">知识库</div>
      <div class="card-mobile bg-[var(--surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] mx-4 mb-3 overflow-hidden">
        <div class="row-item flex items-center justify-between px-4 py-3.5 border-b border-[var(--border)] cursor-pointer active:bg-[var(--bg)]" @click="navigateTo('/knowledge')">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#e8fdf5] flex items-center justify-center text-[15px] shrink-0">📚</div>
            <div>
        ...
        <div class="row-item flex items-center justify-between px-4 py-3.5 cursor-pointer" @click="toggleMemory">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#fff8e6] flex items-center justify-center text-[15px] shrink-0">🧠</div>
            <div>
              <div class="text-sm font-bold text-[var(--text1)]">记忆</div>
              <div class="text-xs text-[var(--text3)] mt-px">自动学习对话，形成个性化知识</div>
            </div>
          </div>
          <div class="toggle-mobile w-[46px] h-[26px] rounded-[13px] relative cursor-pointer transition-colors duration-250 shrink-0" :class="memoryEnabled ? 'bg-[var(--teal)]' : 'bg-[var(--bg2)]'">
            <div class="absolute w-5 h-5 rounded-full bg-white top-[3px] shadow-[0_1px_4px_rgba(0,0,0,.2)] transition-transform duration-250" :class="memoryEnabled ? 'translate-x-5' : 'left-[3px]'"></div>
          </div>
        </div>
      </div>

      <div class="section-label text-xs font-extrabold text-[var(--text3)] tracking-[.8px] uppercase pt-4 pb-2 px-4">账号与设置</div>
      <div class="card-mobile bg-[var(--surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] mx-4 mb-3 overflow-hidden">
        <div class="row-item flex items-center justify-between px-4 py-3.5 border-b border-[var(--border)] cursor-pointer active:bg-[var(--bg)]" @click="ui.showToast('功能开发中…')">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#e8fdf5] flex items-center justify-center text-[15px] shrink-0">🔒</div>
            <div class="text-sm font-bold text-[var(--text1)]">隐私设置</div>
          </div>
        ...
        <div class="row-item flex items-center justify-between px-4 py-3.5 border-b border-[var(--border)] cursor-pointer">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#fff8e6] flex items-center justify-center text-[15px] shrink-0">🔔</div>
            <div class="text-sm font-bold text-[var(--text1)]">消息通知</div>
          </div>
        ...
        <div class="row-item flex items-center justify-between px-4 py-3.5 cursor-pointer active:bg-[var(--bg)]" @click="ui.showToast('当前版本 v4.0.0')">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-[10px] bg-[#eef0fc] flex items-center justify-center text-[15px] shrink-0">ℹ️</div>
            <div>
              <div class="text-sm font-bold text-[var(--text1)]">关于明境</div>
              <div class="text-xs text-[var(--text3)] mt-px">版本 v{{ user.profile.version }}</div>
            </div>
